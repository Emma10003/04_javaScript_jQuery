...
$(function () {
    ...
    $("#로그인기능").click(function (e) {
        ...
        const abc = $("#username").val(); // username 안에 소비자가 작성한 값(id)이 들어있음.
                ...
                */
                if (data.users[abc]) {  // 사용자(아이디)가 존재하는가? 
                    ...
                    if (data.users[abc].password === password) { 
                        ...
                        // 2. 아이디가 존재하지만, 비밀번호가 일치하지 않을 때
                        $("#result").html(
                            "일치하는 아이디나 비밀번호가 없습니다."
                        );
                    }
                } else { // 3. 아이디와 비밀번호 모두 존재하지 않을 때
                    $("#result").html("존재하는 아이디가 없습니다.");
                }
            },
            error: function () {
                alert("데이터 가져오는 데 실패했습니다.");
            },
        });
    });
    ...
    $("#로그아웃기능").click(function (e) {
        // 로그아웃을 진행할 경우
        e.preventDefault();  // 로그아웃 button 태그 내부에 type = submit으로 설정되어 있어, 제출 방지.
            // -> 이걸 안 하는 경우 로그아웃 버튼을 눌렀을 때 창이 새로고침됨.
        $("#로그인기능").show(); // 로그아웃기능 버튼 hide 숨기기 설정
        $("#로그아웃기능").hide(); // 로그인기능 show 보여주기
        $("#username").val(""); // 입력창의 값 초기화
        $("#password").val(""); 
...
//             // 2. 데이터가 사용자가 작성한 아이디 비밀번호와 일치하지 않기 때문에
//             // 데이터를 가져올 수 없을 경우 -> '일치하지 않는 고객이 존재하지 않습니다.'
